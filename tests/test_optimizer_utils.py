...
def test_copy_optimizer_state_sparse_adam():
    ...
    for p_source, p_target in zip(model_source.parameters(), model_target.parameters()):
        if p_source.requires_grad:
            ...
            # Check step count
            assert torch.equal(
                source_state["step"], target_state["step"]
            ), "Step count mismatch"

            # Check exp_avg
            assert torch.allclose(
                source_state["exp_avg"], target_state["exp_avg"]
            ), "First moment (exp_avg) mismatch"

            # Check exp_avg_sq
            assert torch.allclose(
                source_state["exp_avg_sq"], target_state["exp_avg_sq"]
...
def test_copy_optimizer_state_mismatched_param_groups():
    ...
    assert "Cannot copy between optimizers with different param group sizes" in str(
        excinfo.value
    )
