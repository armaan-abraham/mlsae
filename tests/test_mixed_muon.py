...
def test_mixed_muon_optimizer_state_copying():
        ...
        if "optimizer_type" in group and group["optimizer_type"] == "muon":
            if "update_buffer" in group:
                ...
                for i, view in enumerate(group["update_buffer_views"]):
                    assert torch.allclose(view, group["update_buffer"][i]), \
    ...
    for group_idx, (group_source, group_target) in enumerate(zip(opt_source.param_groups, opt_target.param_groups)):
        for p_idx, (p_source, p_target) in enumerate(zip(group_source["params"], group_target["params"])):
            if p_source.requires_grad:
                ...
                for key in source_state:
                    if torch.is_tensor(source_state[key]):
                        ...
                        assert torch.allclose(source_state_copy[param_id][key], target_state[key]), \
    ...
    for group_idx, (group_source, group_target) in enumerate(zip(opt_source.param_groups, opt_target.param_groups)):
        if "optimizer_type" in group_target and group_target["optimizer_type"] == "muon":
            assert group_target["lr"] == 0.01, f"Muon lr was incorrectly copied in group {group_idx}"
            assert group_target["weight_decay"] == 0.01, f"Weight decay was incorrectly copied in group {group_idx}"
            assert group_target["momentum"] == 0.95, f"Momentum was incorrectly copied in group {group_idx}"
        elif "optimizer_type" in group_target and group_target["optimizer_type"] == "adam":
            assert group_target["lr"] == 0.001, f"Adam lr was incorrectly copied in group {group_idx}"
            assert group_target["weight_decay"] == 0.01, f"Weight decay was incorrectly copied in group {group_idx}"
    ...
    for group_idx, (group_source, group_target) in enumerate(zip(opt_source.param_groups, opt_target.param_groups)):
        if "optimizer_type" in group_source and group_source["optimizer_type"] == "muon":
            if "update_buffer" in group_source and "update_buffer" in group_target:
                ...
                test_tensor = torch.tensor(test_value, device=group_target["update_buffer"].device, dtype=buffer_dtype)
                assert torch.allclose(group_target["update_buffer"], test_tensor), \
                ...
                for i, view in enumerate(group_target["update_buffer_views"]):
                    ...
                    test_tensor = torch.tensor(test_value, device=view.device, dtype=view.dtype)
                    assert torch.allclose(group_target["update_buffer"][i], test_tensor), \
        ...
        for p_idx, p in enumerate(group["params"]):
            if p.requires_grad:
                original_state_refs[(group_idx, p_idx)] = {
                    key: id(value) for key, value in opt.state[p].items() 
    ...
    for group_idx, group in enumerate(opt.param_groups):
        ...
        if "optimizer_type" in group and group["optimizer_type"] == "muon":
            if "update_buffer" in group:
                ...
                assert group["update_buffer"].device == device, \
                ...
                test_tensor = torch.tensor(test_value, device=device, dtype=buffer_dtype)
                assert torch.allclose(group["update_buffer"], test_tensor), \
                    f"Update buffer value not preserved for group {group_idx}"
                
                # Verify new buffer was created
                assert id(group["update_buffer"]) != original_buffer_refs[group_idx], \
...
def test_mixed_muon_share_memory():
    ...
    for group_idx, group in enumerate(opt.param_groups):
        ...
        if "optimizer_type" in group and group["optimizer_type"] == "muon":
            if "update_buffer" in group:
                initial_is_shared[(group_idx, "buffer")] = group["update_buffer"].is_shared()
                ...
                for key, value in state.items():
                    if torch.is_tensor(value):
                        assert value.is_shared(), \
                            f"State tensor {key} for param ({group_idx}, {p_idx}) not moved to shared memory"
        
        # Check update buffer for Muon groups
        if "optimizer_type" in group and group["optimizer_type"] == "muon":
            if "update_buffer" in group:
                # Verify buffer is in shared memory
                assert group["update_buffer"].is_shared(), \
