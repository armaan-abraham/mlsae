...
def parse_log(log_file: str) -> None:
    ...
    worker_start_pattern = re.compile(
        r"^Starting worker on device cuda:([0-7])$"
    )  # Support cuda:0 through cuda:7
    worker_waiting_pattern = re.compile(
        r"^GPU worker ([0-7]) waiting for task$"
    )  # Support workers 0-7
    worker_got_task_pattern = re.compile(
        r"^GPU worker ([0-7]) got task$"
    )  # Support workers 0-7
