...
class RLFeatureSelector(nn.Module):
    ...
    def update_selector(self, best_mask):
        ...
        log_probs = torch.log(self.saved_probs + 1e-8) * best_mask + \
                    torch.log(1 - self.saved_probs + 1e-8) * (1 - best_mask)
...
class RLSAE(DeepSAE):
    ...
    def _forward(self, x):
        ...
        if self.training:
            ...
            return loss, act_mag, mse_loss, feature_acts, reconstructed, torch.tensor(0.0)
    
    def forward(self, x):
        loss, act_mag, mse_loss, feature_acts, reconstructed, selector_loss = self._forward(x)
